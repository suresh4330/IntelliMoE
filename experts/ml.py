...
experts/ml.py
-------------
MLExpert — classical machine learning domain.

Updated to use the Google Gemini API instead of the local TinyLlama model
to provide higher-quality ML advice, workflows, and solutions.
"""
...
class MLExpert(BaseExpert):
    """Expert for supervised/unsupervised learning, feature engineering, and ML workflows."""
    ...
    def generation_config(self) -> GenerationConfig:
        return EXPERT_CONFIGS["ml"]

    def answer(self, question: str, memory: "Optional[ConversationMemory]" = None) -> str:
        """
        Generate a machine-learning expert answer using the Gemini API.
        ...
        logger.info("MLExpert: sending request to Gemini API...")
        ...
        try:
            ...
            logger.info("MLExpert: successfully generated response from Gemini API.")
            return response

        except Exception as exc:
            logger.exception("MLExpert failed to generate answer using Gemini API.")
            raise RuntimeError(
                f"MLExpert failed to generate an answer. "
